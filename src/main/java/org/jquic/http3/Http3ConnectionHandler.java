...
class Http3ConnectionHandler implements QuicApplicationProtocolConnectionHandler {
    ...
    @Override
    public void onNewServerStreamAllocated(long streamId, DataOutputStream outputStream, QuicConnectionControl.StreamType streamType) {
        // Do nothing;
    }
    ...
    public void onNewClientStreamAllocated(long streamId, @NonNull QuicConnectionControl control, DataOutputStream outputStream, QuicConnectionControl.StreamType streamType) {
        ...
        logger.debug("New HTTP/3 stream {} allocated on connection {} (quicType={}, h3Role={})",
                    streamId, connectionId, streamType, initialRole);

        Http3StreamContext context = new Http3StreamContext(streamId, initialRole);
        streams.put(streamId, context);
        outs.put(streamId, outputStream);
    }
    ...
    @Override
    public void setOutgoingDatagramStream(DataOutputStream outputStream) {
        //No datagrams in Http3;
    }
    ...
    private void remove(long streamId) {
        streams.remove(streamId);
        try {
            outs.remove(streamId).close();
        } catch (Exception _) {}
    }
    ...
    private static class Http3StreamContext {
        ...
        ParsedFrame pollFrame() {
            ...
            if (view == null) {
                view = ByteBuffer.allocate(8);
            }

            ByteBuffer chunk = chunks.peek();

            while (view.hasRemaining() && chunk != null) {
                int toRead = chunk.position() + Math.min(chunk.remaining(), 8 - view.position());
                view.put(chunk.duplicate().limit(toRead));
                chunk.position(toRead);
                if (chunk.remaining() == 0) chunks.poll();
                chunk = chunks.peek();
            }

            int vpos = view.position();
            int vlim = view.limit();
            long frameType;
            long frameLength;
            try {
                view.flip();
                frameType = QuicVarint.read(view);
                frameLength = QuicVarint.read(view);
            } catch (Exception e) {
                view.limit(vlim).position(vpos);
                return null;
            }

            int headerlen = view.position();

            if (lastlyRecieved < (int) frameLength + headerlen) {
                view.limit(vlim).position(vpos);
                return null;
            }

            byte[] payload = new byte[(int) frameLength];
            int pos = view.remaining();
            view.get(payload, 0, pos);

            while (chunk != null && pos < frameLength) {
                int chunkSize = chunk.remaining();
                int toRead = Math.min((int) frameLength - pos, chunkSize);
                chunk.get(payload, pos, toRead);
                pos += toRead;
            ...
            lastlyRecieved -= (int) (frameLength + headerlen);
