 ...
 */
public class SlicingOutputStream extends OutputStream {
    ...
    private final ChunkedOutputStreamWithAmendments.ChunkConsumer chunkConsumer;
     ...
     */
    public SlicingOutputStream(PoolBuffer pbuf, int chunkSize, int trailingPadding, ChunkedOutputStreamWithAmendments.ChunkConsumer consumer) {
    ...
    public int firstChunkStart() {
        return firstChunkStart;
    }
    ...
    public void write(byte @NonNull [] b, int off, int len) throws IOException {
        ...
        while (bytesWritten < len) {
            buf.limit(buf.capacity());
            int spaceLim = Math.max(0, buf.capacity() - position - trailingPadding);
    ...
    private void triggerCallback() throws IOException {
        if (historyMode) {
            ...
            if (chunkWrappedLen > 0) {
                ...
                chunkSize = chunkConsumer.accept(borrow);
            }

            if (buf.capacity() - position <= advance + trailingPadding) {
                throw  new IllegalStateException("Chunk size is more than buffer size");
            } else {
                position += advance;
            }
    ...
    private boolean atTheBufferEdge() {
        return buf.capacity() - position <= trailingPadding;
    }
    ...
    public void goBack(int logicalPos) {
        if (logicalPos >= logicalOffset) {
            throw new IllegalArgumentException("Invalid position: " + logicalPos + ". Current logical offset is " + logicalOffset);
        }

        LogicalPosition logicalPosition = getLogicalPosition(logicalPos);
        ...
        this.position = logicalPosition.chunkStart + logicalPosition.rpos;
        buf.position(this.position);
        this.chunkStart = logicalPosition.chunkStart;
    }
    ...
    private @NonNull LogicalPosition getLogicalPosition(int logicalPos) {
        int pos = firstChunkStart;
        int logPos = 0;
        for (Map.Entry<Integer, Integer> entry : gaps.entrySet()) {
            int adv = entry.getKey() - pos;
            if (adv > logicalPos - logPos) {
                break;
            }
            logPos += adv;
            pos = entry.getValue();
        }
        return new LogicalPosition(logicalPos - logPos, pos);
    }
    ...
    /**
     * Read all written bytes (without gaps and wrapping)
     * @param logicalPos - logical offset returned by getPos()
     * @return iterator over contignous written chunks starting from logical position to last written.
     */
    public Iterator<ByteBuffer> readyContentFrom(int logicalPos) {
        if (historyMode) throw new IllegalStateException("Cant iterate content while in history mode");

        LogicalPosition lpos = getLogicalPosition(logicalPos);
        ...
        return new Iterator<>() {
            ...
            public ByteBuffer next() {
                ...
                ByteBuffer res;
            if (nextGap == null) {
                res = buf.duplicate().position(curPos).limit(position);
                curPos = position;
            } else {
